 ...
 * so typing an amount does not fire a request per keystroke.
 */

const PRICE_DEBOUNCE_MS = 450;

const CONTROL =
  "w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900 shadow-sm transition-colors focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 focus:outline-none";
...
export default function Filters() {
  ...
  const hasFilters = Boolean(
    city || propertyType || listingType || urlMinPrice || urlMaxPrice,
  );
  ...
  // Keep the price boxes in step with the URL (back button, reset), but never
    ...
    >
      <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
        <Select
          ...
          options={CITIES.map((c) => [c, CITY_LABELS[c]])}
